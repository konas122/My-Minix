...
PRIVATE void init_hd()
{
    ...
    for (int i = 0; i < (sizeof(hd_info) / sizeof(hd_info[0])); i++)
        memset(&hd_info[i], 0, sizeof(hd_info[1]));
...
PRIVATE void hd_open(int device) {
    ...
    // 先判断是否为零，因为`init_hd()`将结构体清零了
    if (hd_info[drive].open_cnt++ == 0) {
        /**
         * 获取硬盘分区表：
         *  读硬盘扇区的工作已封装在`get_part_table()`，和执行`IDENTIFY`命令类似，
         *  执行`READ`命令时我们同样是先填充`hd_cmd`结构，然后交给`hd_cmd_out()`来写寄存器
         */ 
