...
PUBLIC void schedule() 
{
    ...
    // 可以看出，当且仅当 p_flags 为零时，一根进程才可以获得运行的机会。
}
 ...
 * proc.
 * 
 * 每个进程都有自己的LDT，位于进程表的中间，这个函数就是根据LDT中描述符的索引
 * 来求得描述符所指向的段的基地址。
 * 
 ...
 * 
 * 用来由虚拟地址求线性地址，它用到了`ldt_seg_linear()`
 * 
 ...
 * 
 * 该函数用来简单地判断是否发生了死锁。方法是判断消息的发送是否构成一个环，
 * 如果构成环则意味着发生了死锁。
 * 
...
/**
 *#假设有进程A要向进程B发送消息M，该过程会如下：
 *  1. A首先准备好M
 *  2. A通过系统调用sendrec，最终调用msg_send
 *  3. 简单判断是否发生死锁
 *  4. 判断目标进程B是否正在等待来自A的消息
 *      - 如果是：消息会被复制给B，B被解除阻塞，继续运行
 *      - 若不是：A被阻塞，并被加入到B的发送队列中
 * 
 *#假设有进程B要接受消息（来自特定进程、中断或任意进程），该过程会如下：
 *  1. B准备一个空的消息结构体M，最终调用msg_receive
 *  2. B通过系统调用sendrec，最终调用msg_receive
 *  3. 判断B是否有来自硬件的消息（通过has_int_msg），
 *     如果是，并且B准备接受来自中断的消息或准备接受任意消息，
 *     则马上准备一个消息给B，并返回
 *  4. 如果B是想接收来自任意进程的消息，则从自己的发送队列中选取第一个
 *     （若队列非空），将其消息复制给M
 *  5. 如果B像接收来自特定进程A的消息，则先判断A是否正在等待向B发送消息，
 *     若是，则将其消息复制给M
 *  6. 若此时没有任何进程发消息给B，B会阻塞
 */
